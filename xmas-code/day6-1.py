...
def create_node(line):
	return {"parent": line[0],"name": line[1]}

def find_orbits(node, n_list):
	for item in n_list:
		if node[0] == item[1]:
			return 1 + find_orbits(item, n_list)

	return 1

def sum_orbits(n_list):
	orbit_sum = 0

	for node in n_list:
		orbit_sum += find_orbits(node, n_list)

	return orbit_sum

def get_path(item_name, n_list):
	path = [item_name]	

	for item in n_list:
		if item_name == item[1]:
			return path + get_path(item[0], n_list)

	return path

def get_min_path(path_1,path_2):
	for a in range(1,len(path_1)):
		for b in range(1,len(path_2)):
			if path_1[a] == path_2[b]:
				return a + b - 2

print(get_min_path(get_path("SAN", list_of_orbits), get_path("YOU", list_of_orbits)))
