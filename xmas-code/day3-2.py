...
def lay_pipe(grid, instructions, value):
	commands = instructions.split(',')
	pos_y,	pos_x = find_cell(grid, 8)[0]
	...
	for m in commands:
		...
		for i in range(0,int(m[1:])):
			if direction == 'R':
				grid, pos_x, pos_y = move_right(grid, pos_x, pos_y, value)
			elif direction == 'L':
				grid, pos_x, pos_y = move_left(grid, pos_x, pos_y, value)
			elif direction == 'U':
				grid, pos_x, pos_y = move_up(grid, pos_x, pos_y, value)
			elif direction == 'D':
				grid, pos_x, pos_y = move_down(grid, pos_x, pos_y, value)
	#print_grid(grid)
		
def get_closest_intersection(grid):
	centre = find_cell(grid, 8)[0]
	intersections = find_cell(grid, 110)
	closest = -1
	...
	for x in intersections:
		temp = abs(centre[0] - x[0]) + abs(centre[1] - x[1])
...
def smallest_steps_intersection(grid, ins_a, ins_b):
	centre = find_cell(grid, 8)[0]
	intersections = find_cell(grid, 110)
	comb_steps = -1

	for x in intersections:
		a_steps = run_til_point(grid, ins_a, x)
		b_steps = run_til_point(grid, ins_b, x)

		if comb_steps == -1 or comb_steps > (a_steps + b_steps):
			comb_steps = (a_steps + b_steps)

	return comb_steps

def run_til_point(grid, ins, point):
	commands = ins.split(',')
	pos_y,	pos_x = find_cell(grid, 8)[0]
	step = 0
